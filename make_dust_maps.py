#!/usr/bin/env python
#
# make maps of the dust parameters from a BEAST stats file
...
if __name__ == '__main__':
    ...
    # loop through the pixels and generate tehe summary stats
    for i in range(n_x):
        for j in range(n_y):
    ...
    for k, cur_stat in enumerate(sum_stats):
        ...
        # Save to FITS file
        hdu.writeto(stats_filename.replace('stats.fits','map_' 
                                           + cur_stat + '.fits'), 
                    overwrite=True)
